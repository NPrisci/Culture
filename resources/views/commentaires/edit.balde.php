                    ...
                    <form action="{{ route('commentaires.update', $commentaire) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('id_contentu') is-invalid @enderror" 
                                        ...
                                        @foreach($contenus as $contenu)
                                            <option value="{{ $contenu->id_contenu }}" {{ old('id_contentu', $commentaire->id_contentu) == $contenu->id_contenu ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('texte') is-invalid @enderror" 
                                      id="texte" name="texte" rows="5" required>{{ old('texte', $commentaire->texte) }}</textarea>
                            @error('texte')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('note') is-invalid @enderror" 
                                        ...
                                        <option value="1" {{ old('note', $commentaire->note) == '1' ? 'selected' : '' }}>⭐ (1/5)</option>
                                        <option value="2" {{ old('note', $commentaire->note) == '2' ? 'selected' : '' }}>⭐⭐ (2/5)</option>
                                        <option value="3" {{ old('note', $commentaire->note) == '3' ? 'selected' : '' }}>⭐⭐⭐ (3/5)</option>
                                        <option value="4" {{ old('note', $commentaire->note) == '4' ? 'selected' : '' }}>⭐⭐⭐⭐ (4/5)</option>
                                        <option value="5" {{ old('note', $commentaire->note) == '5' ? 'selected' : '' }}>⭐⭐⭐⭐⭐ (5/5)</option>
                                    </select>
                                    @error('note')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
